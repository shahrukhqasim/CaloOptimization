...
G4UserLimits * DetectorConstruction::getLimitsFromDetectorConfig(const Json::Value& data) {
    G4double maxTrackLength = DBL_MAX; // No limit on track length
    G4double maxStepLength = DBL_MAX;  // No limit on step length
    if (not data.empty()) {
        G4double temp = data["limits"]["max_step_length"].asDouble() * m;
        if (temp > 0)
            maxStepLength = temp;
    }
    ...
    if (not data.empty()) {
        G4double temp = data["limits"]["minimum_kinetic_energy"].asDouble() * GeV;
        if (temp > 0)
